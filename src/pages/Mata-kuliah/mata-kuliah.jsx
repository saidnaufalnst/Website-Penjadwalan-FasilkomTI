...
import ButtonMatkul from '../../component/Button/ButtonMatkul'
...
const MataKuliah = () => {
    ...
    const deleteDataMatkul = async (id) => {
        ...
        if (isDeleteData){
            try {
                ...
                const filteredArray = dataMatkul.filter((item) => item.id !== id);
                setDataMatkul(filteredArray);
                setDataChanged(true);
              } catch (error) {
                console.error(error);
              }
    ...
    const editButton = (id) => {
        navigate(`/page/admin/matakuliah/edit-mata-kuliah/${id}`);
    }

    const filteredDataMatkul = dataMatkul.filter((matkul) => {
        return matkul.nama.toLowerCase().includes(searchData.toLowerCase()) || matkul.kode_matkul.toLowerCase().includes(searchData.toLowerCase());
    });
                    ...
                    <form>   
                        <div className="relative justify-start">
                            ...
                            <input 
                                ...
                                className="block h-[45px] w-[368px] p-4 text-sm text-gray-900 border border-gray-400 rounded-lg bg-white" placeholder="Cari Mata Kuliah" />
                        </div>
                    </form>
                    <ButtonMatkul/>
                </div>
                                    ...
                                    {filteredDataMatkul.map((item)=>(
                                        <tr key={item.id} className="border-b text-center text-black text-[13px]">
                                            ...
                                            <td className="flex justify-end gap-3 px-10 py-5">
                                                ...
                                                <div className='flex items-center'>
                                                    <button onClick={() => deleteDataMatkul(item.id)}>
