...
import ButtonConstraint from '../../component/Button/ButtonConstraint';
import axios from 'axios';

const Constraint = () => {
    const [dataConstraint, setDataConstraint] = useState([]);
    ...
    const getDataConstraint = async() =>{
        try{
            const response = await axios.get(`/contraint`);
            const data = response.data.data;
            data.sort((a, b) => a.dosen.name.localeCompare(b.dosen.name));
            setDataConstraint(data);
        }
        catch(error) {
            console.error(error);
        }
    ...
    const deleteDataConstraint = async (id) => {
        const isDeleteData = window.confirm("Apakah Anda yakin ingin menghapus data ini?");

        if (isDeleteData){
            try {
                await axios.delete(`/contraint/${id}`);
                const filteredArray = dataConstraint.filter((item) => item.id !== id);
                setDataConstraint(filteredArray);
                setDataChanged(true);
              } catch (error) {
                console.error(error);
              }
    ...
    const editButton = (id) => {
        navigate(`/page/admin/constraint/edit-constraint/${id}`);
    }

    const filteredDataConstraint = dataConstraint.filter((constraint) => {
        return constraint.dosen.name.toLowerCase().includes(searchData.toLowerCase());
    });
                    ...
                    <form>   
                        <div className="relative justify-start">
                            ...
                            <input 
                                ...
                                className="block h-[45px] w-[368px] p-4 text-sm text-gray-900 border border-gray-400 rounded-lg bg-white" placeholder="Cari Constraint" />
                        </div>
                    </form>
                    <ButtonConstraint/>
                </div>
                    ...
                    <div className="w-full h-full bg-white border border-gray-200 rounded-br-2xl rounded-tl-2xl table-style overflow-hidden">
                        <div className="overflow-hidden w-full h-full card">
                            <table className="w-full">
                                ...
                                <tbody className="">
                                    {filteredDataConstraint.map((item)=>(
                                        <tr key={item.id} className="border-b text-center text-black text-[13px]">
                                            ...
                                            <td className="flex justify-end gap-3 px-10 py-5">
                                                ...
                                                <div className='flex items-center'>
                                                    <button onClick={() => deleteDataConstraint(item.id)}>
