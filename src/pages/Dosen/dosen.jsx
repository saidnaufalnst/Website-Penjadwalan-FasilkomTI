...
import ButtonDosen from '../../component/Button/ButtonDosen'
...
const Dosen = () => {
    ...
    const deleteDataDosen = async (id) => {
        const isDeleteData = window.confirm("Apakah Anda yakin ingin menghapus data ini?");

        if (isDeleteData){
            try {
                await axios.delete(`dosen/${id}`);
                const filteredArray = dataDosen.filter((item) => item.id !== id);
                setDataDosen(filteredArray);
                setDataChanged(true);
              } catch (error) {
                console.error(error);
              }
    ...
    const editButton = (id) => {
        navigate(`/page/admin/dosen/edit-dosen/${id}`);
    }

    const filteredDataDosen = dataDosen.filter((dosen) => {
        return dosen.name.toLowerCase().includes(searchData.toLowerCase()) || dosen.nip.toLowerCase().includes(searchData.toLowerCase());
    });
        ...
        <div className="flex justify-center h-[70%]">
            <div className="w-[90%]">
                    ...
                    <form>   
                        <div className="relative justify-start">
                            ...
                            <input 
                                ...
                                className="block h-[45px] w-[368px] p-4 text-sm text-gray-900 border border-gray-400 rounded-lg bg-white" placeholder="Cari Dosen" />
                        </div>
                    </form>
                    <ButtonDosen/>
                </div>
                ...
                <div className="flex justify-center items-center w-full h-full">
                    <div className="bg-white border border-gray-200 rounded-br-2xl rounded-tl-2xl table-style overflow-hidden w-full h-full">
                        <div className="overflow-hidden w-full h-full card">
                            <table className="w-full">
                                <thead className="sticky top-0 z-50 bg-[#03965C]">
                                    <tr className="text-white text-center border-b text-[15px]">
                                        ...
                                        <th className="py-5 px-[52px] w-[250px] text-right font-medium">
                                            Aksi
                                        </th>
                                    </tr>
                                </thead>
                                <tbody className="w-[90%]">
                                    {filteredDataDosen.map((item)=>(
                                        <tr key={item.id} className="border-b text-center text-black text-[13px]">
                                            ...
                                            <td className="flex justify-end gap-3 px-10 py-5">
                                                ...
                                                <div className="flex items-center">
                                                    <button onClick={() => deleteDataDosen(item.id)}>
