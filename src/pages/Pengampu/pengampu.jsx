...
import ButtonPengampu from '../../component/Button/ButtonPengampu';
...
const Pengampu = () => {
    ...
    const getDataPengampu = async() =>{
        try{
            ...
            data.sort((a, b) => {
                const kodeMatkulCompare = a.matakuliah.kode_matkul.localeCompare(b.matakuliah.kode_matkul);
              
                if (kodeMatkulCompare === 0) {
                  return a.kelas.nama.localeCompare(b.kelas.nama);
                }
              
                return kodeMatkulCompare;
            });
    ...
    const deleteDataPengampu = async (id) => {
        const isDeleteData = window.confirm("Apakah Anda yakin ingin menghapus data ini?");

        if (isDeleteData){
            try {
                await axios.delete(`/pengampu/${id}`);
                const filteredArray = dataPengampu.filter((item) => item.id !== id);
                setDataPengampu(filteredArray);
                setDataChanged(true);
              } catch (error) {
                console.error(error);
              }
    ...
    const editButton = (id) => {
        navigate(`/page/admin/pengampu/edit-pengampu/${id}`);
    }

    const filteredDataPengampu = dataPengampu.filter((pengampu) => {
        return pengampu.dosen.name.toLowerCase().includes(searchData.toLowerCase()) || pengampu.matakuliah.kode_matkul.toLowerCase().includes(searchData.toLowerCase()) || pengampu.kelas.nama.toLowerCase().includes(searchData.toLowerCase());
    });
                    ...
                    <form>   
                        <div className="relative justify-start">
                            ...
                            <input 
                                ...
                                className="block h-[45px] w-[368px] p-4 text-sm text-gray-900 border border-gray-400 rounded-lg bg-white" placeholder="Cari Pengampu" />
                        </div>
                    </form>
                    <ButtonPengampu/>
                </div>
                                    ...
                                    {filteredDataPengampu.map((item) =>(
                                        <tr key={item.id} className="border-b text-center text-black text-[13px]">
                                            ...
                                            <td className="flex justify-end gap-3 px-10 py-5">
                                                ...
                                                <div className='flex items-center'>
                                                    <button onClick={() => deleteDataPengampu(item.id)}>
