...
import ButtonKelas from '../../component/Button/ButtonKelas'
...
const Kelas = () => {
    ...
    const deleteDataKelas = async (id) => {
        const isDeleteData = window.confirm("Apakah Anda yakin ingin menghapus data ini?");

        if (isDeleteData){
            try {
                await axios.delete(`/kelas/${id}`);
                const filteredArray = dataKelas.filter((item) => item.id !== id);
                setDataKelas(filteredArray);
                setDataChanged(true);
              } catch (error) {
                console.error(error);
              }
    ...
    const editButton = (id) => {
        navigate(`/page/admin/kelas/edit-kelas/${id}`);
    }

    const filteredDataKelas = dataKelas.filter((kelas) => {
        return kelas.nama.toLowerCase().includes(searchData.toLowerCase()) || kelas.semester === parseInt(searchData, 10);
    });
                    ...
                    <form>   
                        <div className="relative justify-start">
                            ...
                            <input 
                                ...
                                className="block h-[45px] w-[368px] p-4 text-sm text-gray-900 border border-gray-400 rounded-lg bg-white" placeholder="Cari Kelas" />
                        </div>
                    </form>
                    <ButtonKelas/>
                </div>
                                    ...
                                    {filteredDataKelas.map((item)=>(
                                        <tr key={item.id} className="border-b text-center text-black text-[13px]">
                                            ...
                                            <td className="flex justify-end gap-3 px-10 py-5">
                                                ...
                                                <div className="flex items-center">
                                                    <button onClick={() => deleteDataKelas(item.id)}>
